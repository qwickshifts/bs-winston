...
[@mel.module "winston"]
external createLoggerExternal: config => u = "createLogger";
[@mel.send] external logExternal: (u, Js.Json.t) => unit = "log";
...
let create = (~level=?, ~format=?, ~transports, ~silent=?, ~errorKey=?, ()) => {
  logger:
    createLoggerExternal(
      config(
        ~level?,
        ~format?,
        ~transports=Belt.List.toArray(transports),
        ~silent?,
        (),
      ),
    ),
...
let withJson = (obj, key, json) => {
  logger: obj.logger,
  content: {
    ...obj.content,
    json: [(key, json), ...obj.content.json],
  },
};
...
let jsExnToJson: Js.Exn.t => Js.Json.t =
  jsError => {
    ...
    Js.Exn.name(jsError)
    ->Belt.Option.map(name => Js.Dict.set(e, "name", Js.Json.string(name)))
    ->ignore;
    Js.Exn.message(jsError)
    ->Belt.Option.map(message =>
        Js.Dict.set(e, "message", Js.Json.string(message))
      )
    ->ignore;
    Js.Exn.stack(jsError)
    ->Belt.Option.map(stack =>
        Js.Dict.set(e, "stack", Js.Json.string(stack))
      )
    ->ignore;
    Js.Exn.fileName(jsError)
    ->Belt.Option.map(fileName =>
        Js.Dict.set(e, "fileName", Js.Json.string(fileName))
      )
    ->ignore;
    Js.Json.object_(e);
  };
...
let log = obj => {
  ...
  Belt.Option.map(obj.content.error, error =>
    Js.Dict.set(m, obj.content.errorKey, exnToJson(error))
  )
  |> ignore;
  Js.Dict.set(m, "message", Js.Json.string(obj.content.message));
  Belt.List.forEach(obj.content.json, ((key, json)) =>
    Js.Dict.set(m, key, json)
  );
...
let logErrorExn = (logger, msg, e) =>
  logger->error->withMessage(msg)->withExn(e)->log;
